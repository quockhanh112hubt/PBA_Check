        ...
        try:
            with conn.cursor() as cursor:
                query = """
                    ...
                    FROM (
                        SELECT PBA_ID, TOTAL_JUDGMENT, WORK_TIME, 
                            RANK() OVER (PARTITION BY PBA_ID ORDER BY WORK_TIME DESC) AS RNK
                        FROM [ITMV_KTNG_DB].dbo.AFA_P140_PBA_FUNCTION_HISTORY
                        WHERE PBA_ID = ?
                    ) A 
                    WHERE A.RNK = 1
                """
...
def execute_query_oracle(pba_id):
    ...
    try:
        ...
        query = """
                ...
                FROM (
                    SELECT PBA_ID, TOTAL_JUDGMENT, TRANS_TIME, 
                        RANK() OVER (PARTITION BY PBA_ID ORDER BY TRANS_TIME DESC) AS RNK
                    FROM MIGHTY.ADC_P140_PBA_FUNCTION_HISTORY
                    WHERE PBA_ID = :pba_id
                ) A 
                WHERE A.RNK = 1
        """
...
def create_gui_P1(create_login_ui, create_gui_P230, create_gui_P4):
    ...
    def show_filter_menu(event):
        ...
        # Create popup menu
        filter_menu = tk.Menu(root_P1, tearoff=0)
    ...
    def check_pba_id(event=None):
        ...
        if not pba_id:
            messagebox.showwarning("Cảnh báo", "Vui lòng nhập PBA ID!")
            return
        
        # Show loading state
        result_label.config(text="Đang kiểm tra...", image='')
        pba_label.config(text="Loading...")
        root_P1.update_idletasks()
    ...
    # ==================== MAIN WINDOW ====================
    root_P1 = tk.Tk()
    root_P1.title("PBA Function Checker - Model P1 4.0")
    root_P1.geometry("1400x900")
    root_P1.configure(bg="#f5f7fa")
    ...
    # ==================== HEADER ====================
    header_frame = tk.Frame(root_P1, bg="#1e3a8a", height=100)
    ...
    subtitle_label = tk.Label(
        header_content,
        text="Model P1 4.0 - Production Testing System",
        font=("Segoe UI", 12),
        bg="#1e3a8a",
        fg="#93c5fd"
    )
    ...
    # ==================== MAIN CONTENT ====================
    main_container = tk.Frame(root_P1, bg="#f5f7fa")
    ...
    # ==================== INPUT CARD ====================
    input_card = ModernCard(left_panel, title="🔍 PBA ID Scanner")
    ...
    hint_label = tk.Label(
        input_content,
        text="💡 Scan or enter PBA ID and press Enter",
        font=("Segoe UI", 9),
        bg="white",
        fg="#6b7280"
    )
    ...
    # ==================== FOOTER ====================
    footer_frame = tk.Frame(root_P1, bg="#f5f7fa", height=40)
    ...
    root_P1.after(1000, check_connections)
